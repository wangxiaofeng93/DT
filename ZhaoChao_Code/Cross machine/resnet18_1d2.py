...
class CNN1(nn.Module):
    def __init__(self, pretrained=False, in_channel=1, num_classes=10):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv1d(1, 16, kernel_size=128,stride=1),  # 32, 24, 24
            nn.BatchNorm1d(16),
            nn.ReLU(inplace=True),
        ...
        self.layer2 = nn.Sequential(
            nn.Conv1d(16, 32, kernel_size=64,stride=1),  # 128,8,8
            nn.BatchNorm1d(32),
            nn.ReLU(inplace=True),
            nn.MaxPool1d(kernel_size=4, stride=1))# 128, 4,4

        self.layer3 = nn.Sequential(
            nn.Conv1d(32, 64, kernel_size=16,stride=1),  # 32, 24, 24
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
        ...
        self.layer4 = nn.Sequential(
            nn.Conv1d(64, 128, kernel_size=3,stride=1),  # 128,8,8
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
            nn.MaxPool1d(kernel_size=2, stride=1)
        )
        self.layer5 = nn.Sequential(
            nn.Conv1d(128, 256, kernel_size=2, stride=1),  # 128,8,8
            nn.BatchNorm1d(256),
            nn.ReLU(inplace=True),
            nn.AdaptiveMaxPool1d(4)
        )
